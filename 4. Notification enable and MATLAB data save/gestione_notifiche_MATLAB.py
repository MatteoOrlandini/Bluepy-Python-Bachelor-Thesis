...
class DefaultDelegate:
	...
	def handleNotification(self, cHandle, data):
		...
		if (cHandle == handle_temp_press):							
			#salvataggio del pacchetto ricevuto (notifica) in una variabile ausiliaria
		...
		if (cHandle == handle_acc_gyr_magn):
			...
			#decodifica dei dati
			#accelerometro (unità di misura: g) con range +/- 16 g e fondo scala pari a +/-2^15 (32678)
			acc_x = acc_x * 32 / 32768			
			acc_y = acc_y * 32 / 32768			
			acc_z = acc_z * 32 / 32768
			...
			gyr_y = gyr_y /10
			gyr_z = gyr_z /10
			#magnetometro (unità di misura: μT) con range +/- 50 Gauss e fondo scala pari a +/-2^15 (32678),
			#i dati ricevuti sono espressi in Gauss, poichè 1 G = 100 μT, i valori sono infine moltiplicati per 100 ed espressi in μT
			magn_x = magn_x * 100 /32768 * 100
			magn_y = magn_y * 100 /32768 * 100
			magn_z = magn_z * 100 /32768 * 100
			print("\t\tTimestamp: {}\n\t\tAccx: {} g\n\t\tAccy: {} g\n\t\tAccz: {} g\n\t\tGyrx: {} dps\n\t\tGyry: {} dps\n\t\tGyrz: {} dps\n\t\tMagnx: {} μT\n\t\tMagny: {} μT\n\t\tMagnz: {} μT\n"
			.format(timestamp2, acc_x, acc_y, acc_z, gyr_x, gyr_y, gyr_z, magn_x, magn_y, magn_z)) 
			#scrittura sul file che registra tutti i dati 
			the_file.write("Valore ricevuto accelerometro, giroscopio e magnetometro: {}\n\t\tTimestamp: {}\n\t\tAccx: {} g\n\t\tAccy: {} g\n\t\tAccz: {} g\n\t\tGyrx: {} dps\n\t\tGyry: {} dps\n\t\tGyrz: {} dps\n\t\tMagnx: {} μT\n\t\tMagny: {} μT\n\t\tMagnz: {} μT\n".format(str(binascii.hexlify(acc_gyr_magn_value), 'ascii').upper(), timestamp2, acc_x, acc_y, acc_z, gyr_x, gyr_y, gyr_z, magn_x, magn_y, magn_z))
			#apertura dei file necessari a MATLAB
			file_acc = open (file_accelerometro_matlab, 'a+')
			file_gyr = open (file_giroscopio_matlab, 'a+')
			file_magn = open (file_magnetometro_matlab, 'a+')
			#file_acc.write ("{}\t{}\t{}\t{}\t{}\n".format(timestamp2, str(time), acc_x, acc_y, acc_z))
			#file_gyr.write ("{}\t{}\t{}\t{}\t{}\n".format(timestamp2, time, gyr_x, gyr_y, gyr_z))
			#file_magn.write ("{}\t{}\t{}\t{}\t{}\n".format(timestamp2, time, magn_x, magn_y, magn_z))
			#scrittura dei file necessari a MATLAB
			file_acc.write ("{}\t{}\t{}\t{}\n".format(timestamp2, acc_x, acc_y, acc_z))
			file_gyr.write ("{}\t{}\t{}\t{}\n".format(timestamp2, gyr_x, gyr_y, gyr_z))
			file_magn.write ("{}\t{}\t{}\t{}\n".format(timestamp2, magn_x, magn_y, magn_z))
...
if __name__ == '__main__':
	...
	file_accelerometro_matlab = "Accelerometro MATLAB.txt"
	file_giroscopio_matlab = "Giroscopio MATLAB.txt"
	file_magnetometro_matlab = "Magnetometro MATLAB.txt"
	...
	#ottengo la caratteristica relativa a temperatura e pressione
	ch_temp_press = conn.getCharacteristics(1, 0xFFFF, "00140000-0001-11e1-ac36-0002a5d5c51b")[0]		
	#ottengo la caratteristica relativa a accelerometro, giroscopio e magnetometro
	ch_acc_gyr_magn = conn.getCharacteristics(1, 0xFFFF, "00e00000-0001-11e1-ac36-0002a5d5c51b")[0]													
	#ottengo l'handle della caratteristica relativa a temperatura e pressione
	#serve capire quale caratteristica è relativa alla notifica ricevuta 
	handle_temp_press = ch_temp_press.getHandle()				
	#ottengo l'handle della caratteristica relativa a accelerometro, giroscopio e magnetometro 
	#serve per capire quale caratteristica è relativa alla notifica ricevuta 
	handle_acc_gyr_magn = ch_acc_gyr_magn.getHandle()
	#ottengo il CCCD della caratteristica relativa a temperatura e pressione
	cccd_temp_press = ch_temp_press.getDescriptors(forUUID=0x2902)[0]
	#ottengo il CCCD della caratteristica relativa a accelerometro, giroscopio e magnetometro
	cccd_acc_gyr_magn = ch_acc_gyr_magn.getDescriptors(forUUID=0x2902)[0]		
