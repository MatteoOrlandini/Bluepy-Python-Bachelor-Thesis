...
if __name__ == '__main__':
	...
	file_lista_servizi_caratteristiche = "Lista dei servizi e caratteristiche SensorTile.txt"
	...
	with open(file_lista_servizi_caratteristiche, 'w') as the_file:
		try:
			for svc in conn.services:
				for i in range (0, 5):
					if (uuid_services[i] in str(svc.uuid)):
						print ("\nUUID servizio : {}\nDescrizione servizio : {}\nPrimo handle servizio: {} = D'{}'\nUltimo handle servizio: {} = D'{}'\n".format(uuid_services[i], service_description[i], hex(svc.hndStart), svc.hndStart, hex(svc.hndEnd),svc.hndEnd))
						the_file.write("\nUUID servizio : {}\nDescrizione servizio : {}\nPrimo handle servizio: {} = D'{}'\nUltimo handle servizio: {} = D'{}'\n".format(uuid_services[i], service_description[i], hex(svc.hndStart), svc.hndStart, hex(svc.hndEnd),svc.hndEnd))
				
				for ch in svc.getCharacteristics():
					characteristic_uuid = ch.uuid
					for i in range (0, len(uuid_characteristics)):
						if (uuid_characteristics[i] in str(characteristic_uuid)):
							print("\n\tUUID caratteristica: {}\n\tNome: {}\n\tDescrizione caratteristica: {}\n\tHandle caratteristica: {} = D'{}'\n\tProprietà: {}\n".format(uuid_characteristics[i], feature_bit_mask[i],feature_description[i], hex(ch.handle),ch.handle, ch.propertiesToString()))
							the_file.write("\n\tUUID caratteristica: {}\n\tNome: {}\n\tDescrizione caratteristica: {}\n\tHandle caratteristica: {} = D'{}'\n\tProprietà: {}\n".format(uuid_characteristics[i], feature_bit_mask[i],feature_description[i], hex(ch.handle),ch.handle, ch.propertiesToString()))
